...
export default function DriversList() {
  ...
  return (
    <div className=" bg-gray-50 px-5">
      <div className="mb-6 py-5">
        ...
        <p className="text-sm text-gray-500">List of all the drivers registered</p>
      </div>
  ...
  <button
    className="flex items-center gap-2 text-blue-600 text-sm font-medium
               max-sm:w-full max-sm:justify-center max-sm:text-xs">
  ...
  <div className="w-full max-w-full overflow-x-auto">
    <table className="min-w-max table-auto w-full">
      <thead className="bg-gray-50 border-b">
        <tr>
          <th className="px-3 py-3 text-left text-sm text-gray-600 font-normal">Name</th>
          <th className="px-3 py-3 text-left text-sm text-gray-600 font-normal">Mobile number</th>
          <th className="px-3 py-3 text-left text-sm text-gray-600 font-normal">Rating</th>
          <th className="px-3 py-3 text-left text-sm text-gray-600 font-normal">Status</th>
          <th className="px-3 py-3 text-left text-sm text-gray-600 font-normal">Wallet balance</th>
          <th className="px-3 py-3 text-left text-sm text-gray-600 font-normal">Total Orders</th>
        </tr>
      </thead>
      ...
      <tbody>
        {drivers.map((driver, index) => (
          <tr key={index} className="border-b last:border-b-0 hover:bg-gray-50">
            ...
            <td className="px-3 py-4 text-sm text-gray-700 min-w-[140px]">
              {driver.wallet}
            </td>

            <td className="px-3 py-4 text-sm text-gray-700 min-w-[120px]">
              {driver.orders}
            </td>
          </tr>
        ))}
